-- ref https://github.com/isseii10/workspace-picker.wezterm
...
function M.show_workspace_selector(window, pane)
	...
	local choices = {}
	local current_idx = 1  -- Default to first item (Create New)
	...
	for _, name in ipairs(wezterm.mux.get_workspace_names()) do
		...
		if current == name then
			current_idx = #choices + 1  -- Set current workspace as default selection
			label = wezterm.format({
				{ Foreground = { Color = colors.workspace_prefix } },
		...
		if name == "default" then
			-- Display default workspace after Create New (position 2)
			table.insert(choices, 2, {
				id = "ws:" .. name,
				label = label,
			})
			if current == name then
				current_idx = 2  -- Update current_idx if default is current
			end
		else
			table.insert(choices, {
				id = "ws:" .. name,
		...
		act.InputSelector({
			action = wezterm.action_callback(function(win, p, id, label)
				...
				if id == "create_new" then
					...
					win:perform_action(
						act.PromptInputLine({
							description = "(wezterm) Create new workspace (leave empty for auto-generated name): ",
							action = wezterm.action_callback(function(window, pane, line)
								local workspace_config = (line and line ~= "") and { name = line } or {}
								window:perform_action(
									act.SwitchToWorkspace(workspace_config),
									pane
								)
							end),
						}),
...
function M.create_workspace_manually()
	return act.PromptInputLine({
		...
		action = wezterm.action_callback(function(window, pane, line)
			local workspace_config = (line and line ~= "") and { name = line } or {}
			window:perform_action(act.SwitchToWorkspace(workspace_config), pane)
		end),
	})
end
